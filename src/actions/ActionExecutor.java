...
public class ActionExecutor {
    public static void actionSelector(Database database,
                                      Action action, Output output) throws IOException {
    ...
    public static void executeCommand(Database database,
                                      Action action, Output output) throws IOException {
    ...
    public static void executeActorQuery(Database database, Action action, Output output) throws IOException {
    ...
    public static void executeMovieQuery(Database database, Action action, Output output) throws IOException {
        switch(action.getCriteria()) {
            case "ratings":
                if (action.getSortType().equals("asc")) {
                    List<String> result = database.getMoviesMap().values().stream().filter(m -> m.calculateShowGrade() > 0).
                            filter(m -> m.isFromYear(action.getFilters().get(0))).filter(m -> m.hasAllGenres(action.getFilters().get(1))).
    ...
    public static void executeSerialQuery(Database database, Action action, Output output) throws IOException {
        switch (action.getCriteria()) {
            case "ratings":
                if (action.getSortType().equals("asc")) {
                    List<String> result = database.getSerialsMap().values().stream().filter(s -> s.calculateShowGrade() > 0).
                            filter(s -> s.isFromYear(action.getFilters().get(0))).filter(s -> s.hasAllGenres(action.getFilters().get(1))).
    ...
    public static void executeUserQuery(Database database, Action action, Output output) throws IOException {
    ...
    public static void executeQuerry(Database database, Action action, Output output) throws IOException {
        /*Calcularea nr of favorites, actor rating si nr of views.*/
